...
export default function Completed({order, count, roleName, handleViewDeliveredDetailOpen, page, rowsPerPage, handleChangePage, handleChangeRowsPerPage}) {
    ...
    {(order.length > 0 ? order : []).slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((data, index) => {      
      return(
        <TableRow key={Math.random()}>
          ...
          <StyledTableCell>{getDateInDDMMYYYY(data.delivered_at)}</StyledTableCell>
