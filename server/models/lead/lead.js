      ...
      if(that.user_id!="admin"){
        ...
        connection.query('select franchise_id from user where id=1 limit 1', function (error, rows, fields) {
          if (!error) {
            ...
            const franchise_id=rows[0].franchise_id;
            connection.changeUser({ database: dbName["prod"] });
            connection.query('select id,lead_id, is_franchise_exist,franchise_id,franchise_name,message, is_active,f_id,created_by,converted_to,converted_by,converted_by_f_id from leads where is_active="1" AND converted_to != 0 AND (f_id="'+franchise_id+'" OR franchise_id="'+franchise_id+'" OR franchise_id=0) order by id desc', function (error, rows, fields) {
              if (!error) {
                resolve(rows);
  ...
  return new Promise(function (resolve, reject) {
    connection.getConnection(function (error, connection) {
      ...
      if(that.filter_id===1){
        if(that.user_id!="admin"){
          ...
          connection.query('select franchise_id from user where id=1 limit 1', function (error, rows, fields) {
            if (!error) {
              ...
              const franchise_id=rows[0].franchise_id;
              connection.changeUser({ database: dbName["prod"] });
              connection.query('select id,lead_id, is_franchise_exist,franchise_id,franchise_name,message, is_active,f_id,created_by from leads where is_active="1" AND converted_to="0" AND f_id="'+franchise_id+'" order by id desc', function (error, rows, fields) {
                if (!error) {
                  console.log(rows);
                  resolve(rows);
              ...
              connection.changeUser({ database: dbName["prod"] });
              connection.query('select id,lead_id, is_franchise_exist,franchise_id,franchise_name,message, is_active,f_id,created_by from leads where is_active="1" AND converted_to="0" AND  f_id="0" order by id desc', function (error, rows, fields) {
                if (!error) {
                  resolve(rows);
        ...
        if(that.user_id!="admin"){
          ...
          connection.query('select franchise_id from user where id=1 limit 1', function (error, rows, fields) {
            if (!error) {
              ...
              const franchise_id=rows[0].franchise_id;
              connection.changeUser({ database: dbName["prod"] });
              connection.query('select id,lead_id, is_franchise_exist,franchise_id,franchise_name,message, is_active,f_id,created_by from leads where is_active="1" AND converted_to="0" AND franchise_id="'+franchise_id+'" order by id desc', function (error, rows, fields) {
                if (!error) {
                  resolve(rows);
        ...
        connection.changeUser({ database: dbName["prod"] });
          connection.query('select id,lead_id, is_franchise_exist,franchise_id,franchise_name,message, is_active,f_id,created_by from leads where is_active="1" AND converted_to="0" AND franchise_id=0 order by id desc', function (error, rows, fields) {
            if (!error) {
              resolve(rows);
