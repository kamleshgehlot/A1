  ...
  return new Promise(function (resolve, reject) {
    connection.getConnection(function (error, connection) {
      ...
      if(that.name.split('_')[1] === 'admin'){
        ...
        connection.query('Select AES_DECRYPT(`password`, \'secret\') AS password, id, franchise_id, name as user_name, user_id, status from user where user_id=? and is_active = ?', values, function (error, rows, fields) {
