...
Enquiry.prototype.postenquiry = function () {
  ...
  return new Promise(function (resolve, reject) {
    ...
    connection.getConnection(function (error, connection) {
      ...
      if (!error) {
        let values = [
          [that.enquiry_id, that.lead_id, that.customer_name, that.contact, that.interested_product_id, that.is_active, that.converted_to, that.created_by]
        ]
        connection.changeUser({ database: dbName.getFullName(dbName["prod"], that.user_id.split('_')[1]) });
        connection.query('INSERT INTO enquiry(enquiry_id, lead_id, customer_name, contact, interested_product_id, is_active, converted_to, created_by) VALUES ?', [values], function (error, rows, fields) {
          if (!error) {
