...
ProductManager.prototype.getStateRelatedRecord = function () {
  ...
  return new Promise((resolve, reject) => {
    connection.getConnection((error, connection) => {
      ...
      if (!error) {
          connection.changeUser({database : dbName.getFullName(dbName["prod"], that.user_id.split('_')[1])});
          // select p.product_id, COUNT(p.product_id) as total_rented from (select CAST(t.product_id AS UNSIGNED) as product_id from (select o.id, o.order_status, SUBSTRING_INDEX(SUBSTRING_INDEX(o.product_id, ',', numbers.id), ',', -1) product_id from status_payment as numbers inner join orders as o on (CHAR_LENGTH(o.product_id) - CHAR_LENGTH(REPLACE(o.product_id, ',', '')) >= numbers.id-1)) as t WHERE t.order_status IN(6,7) GROUP BY t.product_id) as p INNER JOIN delivered_product_detail dpd ON dpd.product_id = p.product_id AND dpd.status = 1 GROUP BY p.product_id ORDER BY p.product_id

          connection.query('select p.product_id, COUNT(p.product_id) as total_rented from (select CAST(t.product_id AS UNSIGNED) as product_id from (select o.id, o.order_status, SUBSTRING_INDEX(SUBSTRING_INDEX(o.product_id, \',\', numbers.id), \',\', -1) product_id from status_payment as numbers inner join orders as o on (CHAR_LENGTH(o.product_id) - CHAR_LENGTH(REPLACE(o.product_id, \',\', \'\')) >= numbers.id-1)) as t WHERE t.order_status IN(6,7) GROUP BY t.product_id) as p INNER JOIN delivered_product_detail dpd ON dpd.product_id = p.product_id AND dpd.status = "'+that.tabValue+'" GROUP BY p.product_id ORDER BY p.product_id', function (error, rows, fields) {
            if(error) {  console.log('Error...', error); reject(error); }
                if(!error){
