...
ProductManager.prototype.changeProductState = function () {
  ...
  return new Promise((resolve, reject) => {
    connection.getConnection((error, connection) => {
      ...
      if (!error) {
          connection.changeUser({database : dbName.getFullName(dbName["prod"], that.user_id.split('_')[1])});
          connection.query('UPDATE `ordered_product` SET status = ? WHERE order_id = ? AND product_id = ? AND is_active = ?',[that.newState, that.orderId, that.productId, 1], function (error, rows, fields) {
            if(error) {  console.log('Error...', error); reject(error); }
                if(!error){
                  resolve(rows)
                }
