...
const Task = function (params) {
  ...
  this.reschedule_date = params.reschedule_date,
  this.is_active= 1;
    ...
    connection.getConnection(function (error, connection) {
      ...
      if (error) {
        ...
        const tastActivityValues = [
          [that.taskInsertId, that.assigned_to, that.assign_to_role, that.activity_description, that.due_date, that.msgId, that.docId, 1, 1, that.created_by]
        ];
        connection.changeUser({database : dbName.getFullName(dbName["prod"], that.user_id.split('_')[1])});
    ...
    connection.getConnection(function (error, connection) {
      ...
      if (error) {
        ...
        if(that.status == 2){
          ...
          const tastActivityValues = [
            [that.taskInsertId, that.assigned_to, that.assign_to_role, that.activity_description, that.due_date, new Date(), that.msgId, that.docId, that.status, 1, that.created_by]
          ];
          connection.changeUser({database : dbName.getFullName(dbName["prod"], that.user_id.split('_')[1])});
          connection.query(`INSERT INTO task_activity(task_id, assign_to, assign_to_role, description, due_date, start_date, message_id, document_id, status, is_active, created_by) VALUES ?`, [tastActivityValues], (error, rows, fields) => {
            if (!error) {
          ...
          const tastActivityValues = [
            [that.taskInsertId, that.assigned_to, that.assign_to_role, that.activity_description, that.due_date, that.start_date, new Date(), that.msgId, that.docId, that.status, 1, that.created_by]
          ];
          connection.changeUser({database : dbName.getFullName(dbName["prod"], that.user_id.split('_')[1])});
          connection.query(`INSERT INTO task_activity(task_id, assign_to, assign_to_role, description, due_date, start_date, reschedule_req_date, message_id, document_id, status, is_active, created_by) VALUES ?`, [tastActivityValues], (error, rows, fields) => {
            if (!error) {
    ...
    connection.getConnection(function (error, connection) {
      ...
      if (error) {
        throw error;
          ...
          const tastActivityValues = [
            [that.taskInsertId, that.lastDataState.assign_to, that.lastDataState.assign_to_role_id, that.activity_description, that.lastDataState.due_date,  that.lastDataState.start_date, that.lastDataState.reschedule_req_date, that.msgId, that.docId, 5, 0, that.created_by]
          ];
          connection.changeUser({database : dbName.getFullName(dbName["prod"], that.user_id.split('_')[1])});
          connection.query(`INSERT INTO task_activity(task_id, assign_to, assign_to_role, description, due_date, start_date, reschedule_req_date, message_id, document_id, status, is_active, created_by) VALUES ?`, [tastActivityValues], (error, rows, fields) => {
            if (!error) {
              const tastActivityValues = [
                [that.taskInsertId, that.assigned_to, that.assign_to_role, that.activity_description, that.reschedule_date, that.msgId, that.docId, 1, 1, that.created_by]
              ];
              connection.changeUser({database : dbName.getFullName(dbName["prod"], that.user_id.split('_')[1])});
              connection.query(`INSERT INTO task_activity(task_id, assign_to, assign_to_role, description, due_date, message_id, document_id, status, is_active, created_by) VALUES ?`, [tastActivityValues], (error, rows, fields) => {
                if (!error) {
...
Task.prototype.reschedule = function () {
  ...
  return new Promise(function (resolve, reject) {
    connection.getConnection(function (error, connection) {
      ...
      if (!error) {
        const tastActivityValues = [
          [that.taskInsertId, that.assigned_to, that.assign_to_role, that.activity_description, that.reschedule_date, that.start_date, that.reschedule_req_date, that.reschedule_date, that.msgId, that.docId, 4, 1, that.created_by]
        ];

        connection.changeUser({database : dbName.getFullName(dbName["prod"], that.user_id.split('_')[1])});
        connection.query(`INSERT INTO task_activity(task_id, assign_to, assign_to_role, description, due_date, start_date, reschedule_req_date, reschedule_date, message_id, document_id, status, is_active, created_by) VALUES ?`, [tastActivityValues], (error, rows, fields) => {
          if (!error) {
            resolve({rows});
          }else{           
            console.log("Error...", error);
